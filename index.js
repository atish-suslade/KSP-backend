...
app.post('/api/getbeatlogs', authorize, async (req, res) => {
  ...
  const query = `select * from beat_logs where user_id in  (SELECT o.id FROM users o JOIN officer_hierarchy oh ON o.id = oh.junior_id WHERE oh.senior_id=$1  AND oh.junior_id != $1)`;
  ...
  try {
    ...
    console.error('Error fetching patrol log:', err);
    res.status(500).json({ error: 'Failed to get patrol log' });
  }
});
app.post('/api/getownnbeatlogs', authorize, async (req, res) => {
  const {id} = req.body;
  const userId = id ||  req.user.id; // Assuming you're using the authorize middleware
...
// --------------------------------------------------------------------------------------------
  app.post('/api/get_cumulative_data',authorize, async (req, res) => {
    const  id= req.body.id || req.user.id
    const {year, ioname, district,unitname,frameworks} = req.body
    console.log({year, ioname, district,unitname,frameworks})
    // console.log(officerName);

    // SQL query with parameterized query
    let SQL = `SELECT ioname,fir_stage,"Month","CrimeHead_Name",year 
                  FROM firdetails WHERE user_id in 
                  (SELECT o.id FROM users o JOIN officer_hierarchy oh ON o.id = oh.junior_id WHERE oh.senior_id=$1  AND oh.junior_id != $1) 
                  `;
    const values = [id,];
    if (year) {
        SQL += ` AND "year" = $${values.length + 1}`;
        values.push(year);
    }

    if (ioname) {
        SQL += ` AND ioname = $${values.length + 1}`;
        values.push(ioname);
    }

    if (district) {
        SQL += ` AND district = $${values.length + 1}`;
        values.push(district);
    }
    if (unitname) {
        SQL += ` AND "UnitName" = $${values.length + 1}`;
    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            res.send(result.rows); // Unauthorized status if user not found
        }
    } catch (error) {
        console.error('Error getting info:', error);
        res.status(500).send({ error: 'Error finding cumulative data' });
    }
});


  app.post('/api/get_initial_filters',authorize, async (req, res) => {
    const  id= req.body.id || req.user.id
    
    // console.log(officerName);

    // SQL query with parameterized query
    const SQL = `WITH relevant_ids AS (
                  SELECT o.id 
                  FROM users o 
                  JOIN officer_hierarchy oh ON o.id = oh.junior_id 
                  WHERE oh.senior_id = $1 AND oh.junior_id != $1
                )
                SELECT 
                  ARRAY_AGG(DISTINCT year::text) AS years,
                  ARRAY_AGG(DISTINCT ioname) AS ionames,
                  ARRAY_AGG(DISTINCT district) AS districts,
                  ARRAY_AGG(DISTINCT "UnitName") AS unitnames
                FROM firdetails
                WHERE user_id IN (SELECT id FROM relevant_ids);
                   `
    const values = [id,]
    try {
        // Executing parameterized query
        ...
        console.error('Error getting info:', error);
        res.status(500).send({ error: 'Error finding filter data' });
    }
});
